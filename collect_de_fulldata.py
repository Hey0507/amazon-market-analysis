"""
collect_de_fulldata.py
==========================
德国站点（DE）全量数据采集脚本
- 类目: Kamera-Mikrofone (nodeIdPath: 562066:571860:331964031:316880011:1195978)
- 月份范围: 2024-04 ~ 2026-03 (最近 24 个月)
- 过滤规则:
  * 月销量 >= 80 (DE 阈值, 与 final_processor.py 保持一致)
  * 排除稳定器 / 捆绑包 / 相机机身 (is_unrelated 同步规则)
- 保存格式: data/DE/{year}.json  (dict: { 'YYYYMM': [items] })
...
# ── 配置 ──────────────────────────────────────────────────────────
MARKETPLACE = "DE"
DE_NODE = "562066:571860:331964031:316880011:1195978"   # Kamera-Mikrofone DE
    ...
    while page <= MAX_PAGES:
        ...
        for item in items:
            ...
            if bad:
                ...
                continue
            if units < DE_MIN_UNITS:
                # 触发销量阈值早停
                print(f"    -> [早停] 发现销量低于阈值产品 ({units} < {DE_MIN_UNITS})，停止此月份后续翻页。")
                page = MAX_PAGES + 1
                break
...
def main():
    print("=" * 60)
    print("  德国站点 (DE) 全量数据采集")
    print(f"  类目: Kamera-Mikrofone")
    print(f"  节点: {DE_NODE}")
    print(f"  月份范围: {MONTHS_TO_FETCH[0]} ~ {MONTHS_TO_FETCH[-1]}")
    print(f"  销量最低阈值: >= {DE_MIN_UNITS} 单位/月")
    print("=" * 60)
    ...
    for year, months in sorted(year_months.items()):
        ...
        for month in months:
            ...
            items, excluded, raw_count = fetch_month(MARKETPLACE, DE_NODE, month)
    ...
    print(f"\n{'='*60}")
    print(f"  德国站数据采集完成！累计有效记录: {grand_total} 条，过滤记录: {grand_excluded} 条")
    print("=" * 60)
